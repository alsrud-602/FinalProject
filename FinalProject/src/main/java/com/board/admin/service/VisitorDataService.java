...
public class VisitorDataService {
    ...
    public void backupAndDeleteOldData() throws IOException {
        LocalDateTime oneMonthAgo = LocalDateTime.now().minusDays(1);//일주일 전 파일 삭제
        ...
        if (!oldData.isEmpty()) {
            ...
            // S3로 업로드
            String s3Key = "visitor-data-backup/" + LocalDateTime.now() + ".json";
    ...
    public void saveVisitorData(VisitorData visitorData) {
    	System.out.println("흠냐링1"+visitorData);
    	repository.save(visitorData);
    	System.out.println("시간이몇시죠?"+visitorData.getVisitorTime());
